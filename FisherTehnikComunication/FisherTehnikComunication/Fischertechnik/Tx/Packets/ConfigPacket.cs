namespace FactoryInterface.Fischertechnik.Tx.Packets
{
    ...
    class ConfigPacket : Packet
    {
        ...
        protected override byte[] ConstructPayload()
        {
            ...
            foreach (var universalInputConfig in UniversalInputs)
            {
                // Digital|Input Mode
                //    v       v
                //    x000 000x

                var digitalMask = universalInputConfig.Digital ? (byte)0x80 : (byte)0x00;
                var inputMask = (byte) universalInputConfig.Mode;
